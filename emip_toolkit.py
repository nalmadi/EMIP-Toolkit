...
def Wilming_dataset(filename, sample_size=216):
    """Import the Wilming dataset

    Parameters
    ----------
    filename : str
        path to Wilming dataset .hdf5 file, e.g. '../../etdb_v1.0.hdf5/'
    ...
    with h5py.File(filename, 'r') as file:
        ...
        participant_id = data["SUBJECTINDEX"][0]
        trial_id = data["trial"][0]

        current_fixation = data["fix"][0]

        line = 0

        data = file["Age study"]
        while count < sample_size and line < len(data["SUBJECTINDEX"]):

            if not participant_id == data["SUBJECTINDEX"][line]:
                subject[participant_id] = Experiment(trial=trials,
                                                     eye_tracker="EyeLink2",
                                                     filetype="hdf")
                count += 1
                trials = []

            if not trial_id == data["trial"][line]:
                image_category = data["trial"][line-1]
                image_number = data["on_image"][line-1]
                trials.append(Trial(trial_id=trial_id,
                                    participant_id=participant_id,
                                    image=f"Stimuli_{image_category}/{image_category}/{image_number}.png",
                                    fixations=fixations,
                                    saccades=None,
                                    blinks=None,
                                    samples=None,
            ...
            if not current_fixation == data["fix"][line]:
                timestamp = data["start"][line-1]
                duration = data["end"][line-1]-data["start"][line-1]
                x_cord = data["x"][line-1]
                y_cord = data["y"][line-1]
                pupil = None

                fixations[current_fixation] = Fixation(trial_id=trial_id,
                                                       participant_id=participant_id,
                                                       timestamp=timestamp,
                                                       duration=duration,
                                                       x_cord=x_cord,
                                                       ...
                                                       token="",
                                                       pupil=pupil)
                current_fixation = data["fix"][line]

            line += 1


        # Code to add last line
        timestamp = data["start"][line - 1]
        duration = data["end"][line - 1] - data["start"][line - 1]
        x_cord = data["x"][line - 1]
        y_cord = data["y"][line - 1]
