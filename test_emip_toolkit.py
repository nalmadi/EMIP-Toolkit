...
def test_AlMadi_Dataset():
    ...
    EMIP = tk.AlMadi_dataset(AlMadiPath.replace("./", "") + '/ASCII/', 8)   # gets the structured data of 8 subjects
    
    assert len(EMIP)==8
    assert EMIP['001'].trial[0].get_subject_id()== AlMadiPath.replace("./", "") + '/ASCII/001'
